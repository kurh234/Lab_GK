...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		if (objectNumber == 1)
		   {
			corkscrew(gl2, 1.5); // korkociag gl2, size
		   }
		   else if (objectNumber == 2 )
		   {
			   Pyramid(gl2, 5, 8);	// piramida gl2, size, N
		   }
	...
	public void corkscrew(GL2 gl2, double size)
	{
		...
		gl2.glBegin(gl2.GL_LINE_STRIP);
		  //ciag punktów spirali
		for(double j=0.1,i = 0; i < 300; i++,j+=1) 
		 {
	...
	public void Pyramid(GL2 gl2, double size, int N)
	{
		gl2.glScaled(size, size, size);
		...
		for (int i = 0; i < N; i++) {
			Triangle(gl2, i - 1, i, N);
        }
		
	}
	
	public void Triangle(GL2 gl2 , float j , float k ,float n )
	{
